...
auto WINAPI re::draw_tile(i32 col, i32 row) -> void {
  auto* hdc = GetDC(exe::window_handle);
  auto tile_value = exe::minefield[row * 32 + col] & 0x1f;
  BitBlt(hdc, col * 16 - 5, row * 16 + 39, 16, 16, exe::tile_bmps[tile_value], 0, 0, SRCCOPY);
